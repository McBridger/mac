...
final class bridgeUITests: XCTestCase {
    ...
    func testWarmStartSkipsSetup() throws {
        ...
        XCTAssertTrue(statusText.waitForExistence(timeout: 5), "Should show main menu content, not setup required")
        
        let advertisingPredicate = NSPredicate(format: "label CONTAINS[c] 'Advertising' OR value CONTAINS[c] 'Advertising'")
        let advertisingExpectation = expectation(for: advertisingPredicate, evaluatedWith: statusText, handler: nil)
        wait(for: [advertisingExpectation], timeout: 10.0)

        // 3. Trigger mock device connection
        DistributedNotificationCenter.default().postNotificationName(
            NSNotification.Name("com.mcbridger.test.connect_device"),
            object: nil,
            userInfo: nil,
        ...
        let connectedPredicate = NSPredicate(format: "label CONTAINS[c] 'Connected' OR value CONTAINS[c] 'Connected'")
        let connectedExpectation = expectation(for: connectedPredicate, evaluatedWith: statusText, handler: nil)
        ...
        let namePredicate = NSPredicate(format: "label == 'Pixel 7 Pro (Mock)' OR value == 'Pixel 7 Pro (Mock)'")
        let nameExpectation = expectation(for: namePredicate, evaluatedWith: deviceName, handler: nil)
    ...
    func testFullSetupFlow() throws {
        ...
        app.launchArguments.append(contentsOf: ["--uitesting", "--cold-start", "--mnemonic-length", "\(mnemonicLength)"])
        app.launch()
        ...
        for (index, word) in words.enumerated() {
            ...
            XCTAssertTrue(field.waitForExistence(timeout: 5), "Field \(index) should appear")
